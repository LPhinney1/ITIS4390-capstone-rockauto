...
export default function AddVehicleButton({ onAdd, launchExternally }: Props) {
  ...
  function handleAdd(vehicle: Vehicle) {
    ...
    try {
      window.dispatchEvent(new CustomEvent("vehicle:added", { detail: v }));
    } catch (e) {
  ...
  function handleClick() {
    if (launchExternally) {
      try {
        window.dispatchEvent(new CustomEvent("vehicle:open-form", { detail: {} }));
      } catch (e) {
  ...
  return (
    <div>
        ...
        <button
          type="button"
          onClick={handleClick}
          className="inline-flex items-center gap-2 rounded-lg px-6 py-3 text-white transition-colors relative bg-[#5b5fc7] flex items-center justify-center hover:bg-[#4a4db5] transition-colors"
        >
      ...
      {open && <VehicleForm onSubmit={handleAdd} onCancel={() => setOpen(false)} />}
            ...
            <div key={v.id} className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-500">{v.engine} {v.nickname ? `• ${v.nickname}` : ""}</div>
                </div>
              </div>
            </div>
          ))}
