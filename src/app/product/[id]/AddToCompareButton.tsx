...
export default function AddToCompareButton({
    ...
    const handleAddToCompare = () => {
        ...
        if (compareIds.length >= 3) {
            alert(
                'You can only compare up to 3 products at a time. Please remove a product first.',
            );
            router.push(`/dashboard/compare/${compareIds.join('/')}`);
            return;
        }
    ...
    return (
        <button
            ...
            className={`flex h-12 items-center justify-center rounded-full bg-gray-900 px-10 text-sm font-semibold tracking-wide text-white shadow hover:bg-black ${className}`}
        >
            COMPARE
        </button>
    );
