...
export default function VehicleFormSlot() {
  ...
  const [initial, setInitial] = useState<Partial<Vehicle> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("vehicle:open-form", onOpen as EventListener);
      window.removeEventListener("vehicle:close-form", onClose as EventListener);
    };
  }, []);
      ...
      {open && (
        <VehicleForm
          ...
          onCancel={() => {
            setOpen(false);
            try {
              window.dispatchEvent(new CustomEvent("vehicle:close-form"));
            } catch (e) {}
          }}
        />
      )}
